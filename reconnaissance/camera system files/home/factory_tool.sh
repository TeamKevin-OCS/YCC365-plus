...
custom_init=`ls $MNTPT/*-custom_init.sh`
...
if [ -n "$custom_init" ]; then
    cp -f $custom_init $USERDIR/custom_init.sh
    if [ $? -ne 0 ]; then
        echo "Copy custom_init.sh failed" > $MNTPT/ERROR.txt
        exit;
    fi
    chmod +x $USERDIR/custom_init.sh
fi
